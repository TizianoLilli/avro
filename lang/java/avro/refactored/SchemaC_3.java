...
public abstract class Schema extends JsonProperties implements Serializable {
  ...
  private static final Map<Type, Supplier<Schema>> PRIMITIVE_FACTORIES = primitiveFactories();

  private static Map<Type, Supplier<Schema>> primitiveFactories() {
    Map<Type, Supplier<Schema>> factories = new EnumMap<>(Type.class);
    factories.put(Type.STRING, StringSchema::new);
    factories.put(Type.BYTES, BytesSchema::new);
    factories.put(Type.INT, IntSchema::new);
    factories.put(Type.LONG, LongSchema::new);
    factories.put(Type.FLOAT, FloatSchema::new);
    factories.put(Type.DOUBLE, DoubleSchema::new);
    factories.put(Type.BOOLEAN, BooleanSchema::new);
    factories.put(Type.NULL, NullSchema::new);
    return Collections.unmodifiableMap(factories);
  }

  /** Create a schema for a primitive type. */
  public static Schema create(Type type) {
    Supplier<Schema> factory = PRIMITIVE_FACTORIES.get(type);
    if (factory == null) {
  ...
  static class Name {
    ...
    void writeName(String currentNamespace, JsonGenerator gen) throws IOException {
      if (name != null)
        gen.writeStringField("name", name);
      if (space != null && !space.equals(currentNamespace)) {
        gen.writeStringField("namespace", space);
      } else if (space == null && currentNamespace != null) { // null within non-null
        gen.writeStringField("namespace", "");
      }
    }

    String getQualified(String defaultSpace) {
  ...
  private static abstract class NamedSchema extends Schema {
    ...
    NamedSchema(Type type, Name name, String doc) {
      super(type);
      this.name = name;
      this.doc = doc;
    ...
    boolean writeNameRef(Set<String> knownNames, String currentNamespace, JsonGenerator gen) throws IOException {
      if (name.name != null) {
    ...
    boolean equalNames(NamedSchema that) {
    ...
    void aliasesToJson(JsonGenerator gen) throws IOException {
  ...
  private static class RecordSchema extends NamedSchema {
    ...
    RecordSchema(Name name, String doc, boolean isError, List<Field> fields) {
      super(Type.RECORD, name, doc);
      this.isError = isError;
      setFields(fields);
    }
  ...
  private static class EnumSchema extends NamedSchema {
    ...
    EnumSchema(Name name, String doc, LockableArrayList<String> symbols, String enumDefault) {
  ...
  private static class UnionSchema extends Schema {
    ...
    UnionSchema(LockableArrayList<Schema> types) {
      super(Type.UNION);
  ...
  private static class FixedSchema extends NamedSchema {
    ...
    FixedSchema(Name name, String doc, int size) {
  ...
  static class LockableArrayList<E> extends ArrayList<E> {
    ...
    LockableArrayList(int size) {
      super(size);
    }

    LockableArrayList(List<E> types) {
      super(types);
    }

    @SafeVarargs
    LockableArrayList(E... types) {
      super(types.length);
